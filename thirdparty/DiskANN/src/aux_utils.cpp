...
//    writer.close();
  ...
  int build_merged_vamana_index(std::string     base_file,
                                ...
                                unsigned R, bool fast, double sampling_rate,
    ...
    if (param_list.size() != 5 && param_list.size() != 6 &&
      ...
      LOG(ERROR)
          << "Correct usage of parameters is R (max degree) "
             ...
             "fast (build index faster but lower search recall)"
             ": optional paramter";
      return -1;
    }
...
// This code should only be available in the "build" DLL.
    ...
    diskann::build_merged_vamana_index<T>(
        data_file_to_use.c_str(), diskann::Metric::L2, L, R, fast, p_val,
  ...
  template DISKANN_DLLEXPORT int build_merged_vamana_index<int8_t>(
      ...
      unsigned R, bool fast, double sampling_rate, double ram_budget,
